...
The site has no framework, package manager, build step, server code, database, cookies or analytics. The planned public URL is `https://lafayette-consulting.us/iriz/`. When the Cloudflare project is ready:

1. connect the public GitHub repository;
2. use no build command;
3. set the output directory to `site` when the project root is the repository root;
4. configure the `/iriz/` production path;
5. publish a signed and notarized GitHub Release asset named exactly `Iriz.zip`; and
6. verify the Home, Download, Privacy, Support, Press Kit and 404 routes after deployment.
